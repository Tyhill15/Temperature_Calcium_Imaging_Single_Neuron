clear; close all
dir_of_files = uigetdir('Analyzed/');
...
bc = zeros(length(GC),240);
for i = 1:length(GC)
    if(mean(find(GC(i).blc > 15)) > 900)
                 bc(i,:) = GC(i).blc(241:1200);
    elseif(mean(find(GC(i).blc > 15)) > 600  && mean(find(GC(i).blc > 15)) <900)
        bc(i,:) = GC(i).blc(121:900);
    else
           if(length(GC(i).blc) < 240)
           bc(i,:) = [GC(i).blc zeros(1,240 - length(GC(i).blc)) ];
           else
           bc(i,:) = GC(i).blc(1:240);
           end
    end
