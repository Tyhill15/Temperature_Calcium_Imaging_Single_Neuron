...
while (ifd_pos ~= 0)
    ...
    for i = 1:num_entries
        ...
        switch TIF.entry_tag
            ...
            case 50838
                IMG.imagejbytecount = 'imagejbytecountnotsupported';
            case 50839
                IMG.imagejmeta = 'imagejmetadatanotsupported';
            case 315
    ...
    if isfield( TIF, 'MM_stack' )
        ...
        if ( img_skip + 1 >= img_first )
            ...
            try
                if(exist('stack'))
                  missing_fields = setdiff(fieldnames(stack),fieldnames(IMG));
                  if(~isempty(missing_fields))
                   for mf = 1:length(missing_fields)
                      IMG = setfield(IMG,missing_fields{mf},'null');
                   end
                       IMG = orderfields(IMG,stack);
