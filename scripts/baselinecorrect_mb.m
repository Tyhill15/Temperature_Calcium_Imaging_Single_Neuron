...
[filename pathname] = uigetfile(['ASI_hold_20/' filesep '*.mat']);
load([pathname, filename], '-mat','delta_F_mean','resampletemp');

scrsz = get(0,'ScreenSize');

fig=figure('Position',[1 scrsz(4) scrsz(3) scrsz(4)],'Color',[1 1 1]);
h  = plot(delta_F_mean);
disp(filename);
...
save([pathname 'blcrrt_'  filename(1:end-4) '.mat'],'blc','delta_F_mean','resampletemp');
